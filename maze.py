...
while True:
    root, dots, vidi, ghosts = init_game(display)
    score = 0
    playing = True
    ...
    while playing:
        ...
        # Move vidi-Man if the next cell is not a wall
        if MAZE[ny][nx] != "X":
            vidi["pos"] = [nx, ny]
            vidi["shape"].x = nx * CELL 
            vidi["shape"].y = ny * CELL 
        ...
        for g in ghosts:
            ...
            for mx, my in dirs:
                ...
                if MAZE[gy + my][gx + mx] != "X":
                    g["pos"] = [gx + mx, gy + my]
                    g["shape"].x = (gx + mx) * CELL 
                    g["shape"].y = (gy + my) * CELL 
                    break

        # Collision detection: vidi-Man meets a ghost
        for g in ghosts:
            if vidi["pos"] == g["pos"]:
